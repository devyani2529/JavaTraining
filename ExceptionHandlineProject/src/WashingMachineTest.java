...
public class WashingMachineTest {
	public static void main(String[] args) {
		...
		Laundry laundry1=washingMachine1.wash(washPowder,water,elect,cloth);
...
class WashingMachine extends Machine { 
	...
	void WashingMachine() throws ClothsOverloadException, DoorNotClosedException{
		
		double value= Math.random()%10;
		
		if(value<0.10) {
			clothsOverload=false;
		}
		else {
			System.out.println("Cloths are sufficient...!!!");
		}
		
		if(clothsOverload==false) {
		...
		if(doorNotClosed==false) {
			DoorNotClosedException ex = new DoorNotClosedException("Cloths are overloaded please remove some cloths...");
			throw ex;
		}
	}
	...
	Laundry wash(WashingPowder washPowder, Water water, Electricity elect, Cloth cloth[]) {
		...
		double value = Math.random()%10;
		if(value>0.96) {
			LowVoltageException ex1 = new LowVoltageException("Oh No!!! Red signal is dishonoured....");
			throw ex1;
		}
		else if(value>0.65 && value <=0.70) {
			WaterSupplyException ex1 = new WaterSupplyException("Oh No!!! tyre got punctured.......");
			throw ex1;
		}
		else if(value>0.65 && value <=0.70) {
			WaterDrainageException ex1 = new WaterDrainageException("Oh No!!! tyre got punctured.......");
			throw ex1;
		}
